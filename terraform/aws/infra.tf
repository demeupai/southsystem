...
resource "aws_key_pair" "south_key_pair" {
...
resource "aws_security_group" "rancher_sg_allowall" {
  ...
  description = "Rancher south - allow all traffic"
...
resource "aws_instance" "rancher_server" {
  ...
  key_name        = aws_key_pair.south_key_pair.key_name
...
module "rancher_common" {
  ...
  workload_cluster_name       = "south-aws-custom"
}

# AWS EC2 instance for creating a single node workload cluster
resource "aws_instance" "south_node" {
  ami           = data.aws_ami.ubuntu.id
  instance_type = var.instance_type
  ...
  user_data = templatefile(
    join("/", [path.module, "files/userdata_south_node.template"]),
    {
      docker_version   = var.docker_version
      username         = local.node_username
